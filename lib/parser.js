...
module.exports = function(program) {
    ...
    var nullParser = consumer(nullRegEx, function(val){
        return {type: "Literal", value: null}
    })

    var booleanParser = consumer(booleanRegEx, function(val){
        return {type: "Literal", value: val}
    })

    var numberParser = consumer(numberRegEx, function(val){
        return {type: "Literal", value: Number(val)}
    })

    var stringParser = consumer(stringRegEx, function(val){
        return {type: "Literal", value: matches[0].slice(1,matches[0].length-1)}
    })

    var literalParser = parserFactory([nullParser, booleanParser, numberParser, stringParser])

    var identifierParser = consumer(identifierRegEx, function(val) {
    ...
    var openParanthesesParser = consumer(openParanthesesRegEx, function(val){
        return true
    })
    
    var closingParanthesesParser = consumer(closingParanthesesRegEx, function(val){
        return true
    })
    ...
    function argumentsParser() {
        ...
        while(!closingParanthesesParser()){
            arg = expressionParser() || elementParser()
            if (!arg)
    ...
    function macroDeclarationParser() {
        ...
        //body
        macro.body = expressionParser(program.slice(0,program.length-1))
    ...
    function functionBodyParser() {
        ...
        while(stmts.length != 0){
            ...
            if (stmts.length > 0) {
                ...
                //Return statement
                expr.push({type: "ReturnStatement", argument: expressionParser()})
            }
        }
        return expr
    }
    ...
    function statementParser() {
        return arrowFunctionParser() || macroDeclarationParser()|| functionDeclarationParser() || variableDeclarationParser() || expressionStatementParser() || elementParser()
    }
