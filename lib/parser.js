...
module.exports = function(program) {
	...
	var expressionParser = function(){
		return callExpressionParser() || binaryExpressionParser() || bodyParser()
	}
	...
	var charCount = function(char, str) {
		var fromIndex = 0, count = 0
		while(str.indexOf(char, fromIndex) != -1){
			count++
			fromIndex = (str.indexOf(char, fromIndex))+1
		}
		return count
	}
	...
	var extractStatements = function(){
		...
		for(var i = 0; i < program.length; i++){
			if(i != 0 && openP == 0){
				var stmt = program.slice(idx, i).trim()
				if(stmt.length > 0)
					stmts.push(stmt)
				idx = i
			}
	...
	var functionBodyParser = function(){
		var stmts = extractStatements()
		var expr = []
		while(stmts.length != 0){
			program = stmts.shift()
			if(stmts.length > 0){
				//Not a return statement
				expr.push(bodyParser())
			}
			else{
				//Return statement
				expr.push({"type": "ReturnStatement", "argument": bodyParser()})
			}

			// if(program.indexOf('(') >= 0 || charCount(')', program) > 1){
			// 	//Not a return statement
			...
			// else{
			// 	//Return Statement
			// 	if(charCount(')', program) == 1){
			// 		stmts.push({"type": "ReturnStatement", "argument": elementParser()})
			// 	}
			// 	else{
			// 		program = '(' + program
