...
var macroKeywordRegEx = /^(\()(\s)*macro\b/
...
module.exports = function(program) {
    ...
    var macroKeywordParser = function() {
        var matches = macroKeywordRegEx.exec(program)
        if(matches === null) return null
        program = program.slice(matches[0].length)
        return matches[0]
    }
    ...
    var dereferencer = function(macroname, tree) {
        for(objs in tree){
            // console.log(objs)
            if(typeof tree[objs] == "object"){
                if(Array.isArray(tree[objs])){
                    tree[objs].forEach(function(t){
                        tree[objs] = [dereferencer(macroname, t)]
                    })
                }
                else{
                    tree[objs] = dereferencer(macroname, tree[objs])
                }
            }
            ...
            if(tree[objs] == "Identifier"){
                if(tilde = /^\~.*\b/.exec(tree.name)){
                    ...
                    var temp = program
                    program = macros[macroname].params[ref]
    ...
    var callExpressionParser = function() {
        ...
        if(expr.callee = identifierParser()){
            ...
            if(macro = macros[expr.callee.name.toString()]){
                ...
                return dereferencer(macroName, macro.body)
    ...
    var macroDeclarationParser = function() {
        if(macroKeywordParser() === null)    return null
