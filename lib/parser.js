...
module.exports = function(program) {
    ...
    var callExpressionParser = function() {
        ...
        if(expr.callee = identifierParser()){
            ...
            if(macro = macros[expr.callee.name.toString()]){
                //params
                var args = expr.arguments.map(function(a){
                    if(typeof a.value == "string")  return '"'+a.value+'"'
                    return a.value
                })
                for(param in macro.params){
                    if(macro.params[param] != "rest"){
                ...
                //body
                var ref = /~.*\b/.exec(macro.body)
                ref = ref[0].slice(1)
                macro.body = macro.body.replace('~'+ref,macros[expr.callee.name.toString()].params[ref])

                temp = program
                program = macro.body
                var macroEval = expressionParser()
                program = temp
    ...
    var macroDeclarationParser = function() {
        ...
        //body
        macro.body = program.slice(0, program.length-1)
