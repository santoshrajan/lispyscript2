        ...
        for(objs in tree){
            if (Array.isArray(tree[objs])) {
                ...
                tree[objs].forEach(function(t, idx){
                    ...
                    dereferencer(macro, t)
                    objs = persistObjs
                    if(Array.isArray(t['deletenode'])){
                        restArgs = t['deletenode']
                    }
                })
            ...
            if (objs == "type" && tree[objs] == "Identifier") {
                if (tilde = /^\~.*\b/.exec(tree.name)) {
                    ...
                    if(ref === "rest"){
                        tree['deletenode'] = []
                        macro.params[ref].forEach(function(refTree){
                            var tempTree = {}
                            for(attr in refTree){
                                tempTree[attr] = refTree[attr]
                            }
                            tree['deletenode'].push(tempTree)
                        })
                    }
                    else{
                        refTree = macro.params[ref]
                         for(attr in refTree){
                            tree[attr] = refTree[attr]
                        }
                    }
                }
            }
        ...
        if (expr.callee = identifierParser()) {
            ...
            if (macroCopy = macros[expr.callee.name.toString()]) {
                ...
                var args = expr.arguments.map(function(arg){
                    /* Fix this: Should be ready for any kind of args, not just a literal*/
                    return arg
                })
                ...
                // console.log(macro.body[0].expression)
                // console.log('------------')
